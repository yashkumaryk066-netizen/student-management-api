...
def generate_student_receipt_pdf(payment):
    ...
    # --- SCHOOL HEADER (Simulated here, logic usually in draw_header_footer) ---
    # We rely on draw_header_footer to paint the background/logo
    
    elements.append(Spacer(1, 20))
    elements.append(Paragraph("FEE RECEIPT", ParagraphStyle('Title', parent=styles['Heading1'], alignment=TA_CENTER, fontSize=24, textColor=colors.HexColor('#1e40af'))))
    elements.append(Spacer(1, 20))
    
    # Receipt Info
    data = [
        [f"Receipt No: REC-{payment.id:06d}", f"Date: {date.today().strftime('%d-%b-%Y')}"],
        [f"Student Name: {payment.student.name.upper()}", f"Class/Grade: {payment.student.grade}"],
        [f"Parent: {payment.student.parent.username if payment.student.parent else 'N/A'}", f"Payment Mode: {payment.status}"]
    ]
    t_info = Table(data, colWidths=[3.5*inch, 3*inch])
    ...
    fee_data = [
        ["DESCRIPTION", "AMOUNT"],
        [payment.description or "Tuition Fees", f"{CURRENCY_SYMBOL} {payment.amount:,.2f}"],
        ["", ""],
        ["TOTAL PAID", f"{CURRENCY_SYMBOL} {payment.amount:,.2f}"]
    ]
    
    t_fee = Table(fee_data, colWidths=[5*inch, 1.5*inch])
