...
class BookIssue(AuditModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            ...
            if old_instance.status == 'ISSUED' and self.status == 'RETURNED':
                self.book.available_copies += 1
                self.book.save()
                self.return_date = timezone.now().date()
            
            # Handle other status changes like LOST (available copies stay reduced)
            # Re-calculating fine if needed
            self.calculate_fine()
